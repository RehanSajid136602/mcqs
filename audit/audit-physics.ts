...
interface AuditResult {
  ...
  question: string;
  marked_correct_index: number;
  marked_correct_answer: string;
  verdict: "WRONG_ANSWER" | "WRONG_EXPLANATION" | "LATEX_ERROR" | "DUPLICATE" | "BAD_DISTRACTOR" | "NEEDS_REVIEW";
  issue: string;
  suggested_correct_index?: number;
  suggested_correct_answer?: string;
  suggested_explanation?: string;
}

// Physics knowledge base for FBISE HSSC-2 (Class 12)
// Chapter mapping from data.json
...
// Correct answers for known Physics MCQs (sample - will be expanded during audit)
// This is a knowledge base for verification
const KNOWN_PHYSICS_FACTS: Record<string, { correct: number; explanation: string }> = {
...
function checkLaTeX(text: string): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  // Check for missing backslashes in common commands
...
// Check if answer is correct based on physics knowledge
function verifyAnswer(question: string, options: string[], markedCorrect: number, chapterTopics: string[]): { 
  isCorrect: boolean; 
  ...
  for (const topic of chapterTopics) {
    if (qLower.includes(topic.toLowerCase())) {
      // Basic sanity checks for common physics questions
      
      // Gravitation chapter checks
  ...
  // If no specific knowledge match, mark as needs review for manual verification
  return { 
    isCorrect: true, 
    confidence: "LOW", 
...
function checkExplanation(question: string, explanation: string | undefined, correctAnswer: string): {
  isValid: boolean;
  issues: string[];
  suggestedFix?: string;
} {
  ...
  // Check if explanation actually addresses the question
  ...
  if (expLower.includes("wrong") || expLower.includes("incorrect") || expLower.includes("not the")) {
    // This might be explaining why other options are wrong - check if it's clear
  ...
  // Check for physics-specific issues
  if (expLower.includes("because") && !expLower.includes("therefore") && explanation.length > 200) {
    // Long explanation with only "because" might be run-on
...
function checkDistractors(options: string[], correctIndex: number, question: string): {
  hasBadDistractors: boolean;
  issues: string[];
} {
  const issues: string[] = [];
  ...
  for (let i = 0; i < options.length; i++) {
    ...
    // Check for obviously nonsensical options
    if (distractor.includes("none of these") || distractor.includes("all of these")) {
      // These are acceptable in some contexts
      continue;
    }
    
    // Check for distractors that are way off in magnitude (for numerical questions)
    ...
    // Check for distractors that contradict basic physics
    if (question.toLowerCase().includes("unit") && distractor.includes("meter") && !correctAnswer.toLowerCase().includes("meter")) {
      // Unit questions need careful distractors
      continue;
    }
  }
...
// Main audit function
function auditPhysics(): void {
  const dataPath = join(process.cwd(), "data", "data.json");
  const data: DataJSON = JSON.parse(readFileSync(dataPath, "utf-8"));
  ...
  const questionTexts = new Map<string, { chapter: string; setId: string; index: number }>();
